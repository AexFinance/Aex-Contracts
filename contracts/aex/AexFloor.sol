...
contract AexFloor is ReentrancyGuard, TokenManager {
    ...
    modifier onlyHandler() {
        require(isHandler[msg.sender], "AexFloor: forbidden");
        _;
    }
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(_backedSupply > backedSupply, "AexFloor: invalid _backedSupply");
        backedSupply = _backedSupply;
    }

    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(_mintMultiplier > mintMultiplier, "AexFloor: invalid _mintMultiplier");
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the AEX tokens to be transferred out must be pre-transferred into this contract
    function mint(uint256 _amount, uint256 _maxCost, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "AexFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "AexFloor: _maxCost exceeded");
        ...
        IERC20(aex).transfer(_receiver, _amount);
    ...
    function burn(uint256 _amount, uint256 _minOut, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        ...
        require(amountOut >= _minOut, "AexFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(aex).burn(msg.sender, _amount);
